...
for file in os.listdir('outputs-reader-pref'):
    with open('outputs-reader-pref/{}'.format(file)) as temp:
    ...
    if num_readers != 0:
        reader_acquire_indices = [find(reader_acquire_str(id), lines) for id in range(2*num_readers)]
        reader_release_indices = [find(reader_release_str(id), lines) for id in range(2*num_readers)]
